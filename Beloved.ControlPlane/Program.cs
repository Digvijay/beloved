...
using Beloved.ControlPlane.Endpoints;
...
// Bind & Validate Options on startup
builder.Services.AddOptions<Beloved.ControlPlane.Options.OciVaultOptions>()
    .Bind(builder.Configuration.GetSection(Beloved.ControlPlane.Options.OciVaultOptions.SectionName))
    .ValidateDataAnnotations()
    .ValidateOnStart();

builder.Services.AddOptions<Beloved.ControlPlane.Options.JwtOptions>()
    .Bind(builder.Configuration.GetSection(Beloved.ControlPlane.Options.JwtOptions.SectionName))
    .ValidateDataAnnotations()
    .ValidateOnStart();

builder.Services.AddOptions<Beloved.ControlPlane.Options.AssemblyEngineOptions>()
    .Bind(builder.Configuration.GetSection(Beloved.ControlPlane.Options.AssemblyEngineOptions.SectionName))
    .ValidateDataAnnotations()
    .ValidateOnStart();
...
var jwtIssuer   = builder.Configuration["Jwt:Issuer"]   ?? "beloved.dev";
var jwtAudience = builder.Configuration["Jwt:Audience"] ?? "beloved.dev";
...
builder.Services.AddScoped<IModuleVerificationService, ModuleVerificationService>();
...
// Register the Vault Repository using the Typed HttpClient pattern
...
// -------------------------------------------------------------------------
// Pluggable LLM Provider Registration
...
// Run EF Core Migrations and Seed Default Tenant
