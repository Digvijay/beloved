...
public static class BelovedDiagnostics
{
    ...
    public static readonly Histogram<double> OciPullDurationHistogram = Meter.CreateHistogram<double>(
        "beloved_oci_pull_duration_seconds",
        unit: "s",
        description: "Latency of OCI layer downloads and extraction");

    public static readonly Counter<long> CacheHitsCounter = Meter.CreateCounter<long>(
        "beloved_cache_hits_total",
        description: "Total L1/L2 Redis vault cache hits");

    public static readonly Counter<long> CacheMissesCounter = Meter.CreateCounter<long>(
        "beloved_cache_misses_total",
        description: "Total L1/L2 Redis vault cache misses");

    public static readonly Counter<long> SecurityRejectionCounter = Meter.CreateCounter<long>(
        "beloved_security_rejections_total",
        description: "Total Roslyn security analyzer code rejections");

    public static readonly Histogram<double> TokenCompressionRatioHistogram = Meter.CreateHistogram<double>(
        "beloved_token_compression_ratio",
        unit: "percent",
        description: "Percentage reduction achieved by AstTokenCompressor");
}
